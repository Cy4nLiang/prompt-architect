...
Three real pipeline runs, three completely different needs — and three **structurally different** prompt skeletons (this anti-homogenization is the suite's core selling point). Full result pages live in [`examples/`](examples/) (download and open in a browser).

| Input (user's words) | Genre detected | Candidates | Winner strategy | Judge score |
|---|---|---|---|---|
| "Write me a prompt for 小红书-style product-seeding posts, bestie-recommendation voice" | creative | 3 | persona-driven | 0.87 ✅ |
| "Write a system prompt for our e-commerce customer-service bot — empathetic, never over-promise compensation" | conversation_role | 3 | dialogue-scripted | 0.88 ✅ |
| "Write a prompt that extracts party A/B, amount, signing date, breach clauses from contracts, for database ingestion" | extract (machine-consumed) | 1 (deliberately) | contract-anchored | 0.93 ✅ |
...
[![Result page — 3-candidate grid](examples/screenshots/shot-xiaohongshu.png)](examples/result-xiaohongshu.html)

<details>
<summary>📸 The other two result pages</summary>

[![Customer-service bot](examples/screenshots/shot-cs-bot.png)](examples/result-cs-bot.html)

[![Contract extraction](examples/screenshots/shot-contract.png)](examples/result-contract-extract.html)

</details>

### Try it yourself — three English starter templates

The example pages above were produced from Chinese inputs (the suite is bilingual). Paste any of these into Claude Code to reproduce the same pipelines in English:
...
**Docs are Chinese-heavy — will it work for me?**
Yes. Skills operate in whatever language you use; ask in English, get English prompts. The [Chinese README](README.md) has extra material (an onboarding guide for non-technical colleagues, knowledge-base HTML).
