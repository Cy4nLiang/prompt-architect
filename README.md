...
[![PRs Welcome](https://img.shields.io/badge/PRs-welcome-7c9cff.svg)](README.en.md#contributing)
[![Claude Code](https://img.shields.io/badge/Claude%20Code-skill%20suite-46e0d0.svg)](https://claude.com/claude-code)

**A prompt optimizer & compiler suite for Claude Code** — mines your real intent into a typed IR, compiles **genre-aware** structured prompts (6 distinct skeletons, no one-size-fits-all), generates 3 candidates scored by an LLM-judge, and ships an interactive HTML report. Covers **text**, **image** (Midjourney / Seedream / gpt-image…) and **video** (Seedance / Sora / Kling / Veo…) prompts.

```bash
# Quick start (requires Claude Code)
git clone https://github.com/Cy4nLiang/claude-code-prompt-architect.git
cd /path/to/your-project && /path/to/claude-code-prompt-architect/install.sh --project
# then restart Claude Code and just say: 帮我优化这个 prompt / "rewrite this prompt"
```

> 📖 **[Full English documentation →](README.en.md)** — docs below are in Chinese; the skills themselves work in any language.
...
**Q：怎么确认 skill 装上了？**
在你的项目里执行 `ls .claude/skills`，应看到 `prompt-architect` 加 6 个 `pa-*` 目录；然后**重启 Claude Code**（skill 在会话启动时加载），直接说需求即可。没触发就说得更直白些："用 prompt-architect 优化：…"。
