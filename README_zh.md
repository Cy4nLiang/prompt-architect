...
[![PRs Welcome](https://img.shields.io/badge/PRs-welcome-7c9cff.svg)](README.md#contributing)
...
> 给开发者另备一条 **coding 专线**（`pa-coding`，唯一可直接调用的子 skill）：自动读 repo 补上下文、几道选择题问清验收与边界，编译出最佳任务 prompt，确认后直接在当前 Claude Code 会话开工。
...
**什么是 skill？** Claude Code（Anthropic 官方 CLI/桌面端）支持给 AI 装"技能包"——放在 `.claude/skills/` 目录下的一组说明书。装好后不需要任何代码，AI 会在合适的时机自动按说明书工作。本仓库就是 8 个这样的技能包。
...
帮我写个产品广告视频的 prompt
帮我把这个需求整理成最佳 prompt 再开工：给登录接口加限流
```
...
8 个 skill 组成一条流水线，**只有 `prompt-architect` 是入口**，其余都是它的内部工序（不要直接喊它们的名字——唯一例外是 coding 专线 `pa-coding`，可直接调用）：
...
**线 4 · coding 专线（`pa-coding`，可直接调用——轻量独立，终点是执行而非报告）**：

```mermaid
flowchart LR
    A["你的开发任务"] --> P["pa-coding<br/>自动读 repo 补上下文<br/>+ 几道选择题咨询"]
    P --> B["任务 brief<br/>（目标/验收/<br/>scope/验证）"]
    B --> C{"确认开工？"}
    C -->|确认| X["当前 Claude Code<br/>会话直接执行"]
    C -->|微调| P
```
...
| pa-coding | coding 专线（可直接调用）：自动读 repo 补上下文，选择题问清验收与 scope，编译任务 brief，确认后本会话直接开工 |
...
└── pa-coding/                       ← coding 专线（reference/ 任务 brief 模板 + 分型咨询问题库）

evals/trigger-cases.yaml             ← 15 条触发回归用例（改 skill description 前后各跑一遍）
...
不行也不必。pa-* 都是内部工序，绕过入口会跳过防注入围栏和意图挖掘——这正是产出同质化、被注入带偏的根源。直接说需求即可，路由是自动的。（唯一例外是 `pa-coding`：coding 专线就是为直接调用设计的。）

**Q：怎么确认 skill 装上了？**
在你的项目里执行 `ls .claude/skills`，应看到 `prompt-architect` 加 7 个 `pa-*` 目录；然后**重启 Claude Code**（skill 在会话启动时加载），直接说需求即可。没触发就说得更直白些："用 prompt-architect 优化：…"。
